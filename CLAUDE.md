...
│       │   │   ├── MatchMomentumFlow.tsx  # V5 Sprint 32 — "The Live Pressure Cooker": dual-gradient area SVG built from lib/matchBoxscore.ts's DELTA-based pressure samples (a genuinely different, legitimately smoothable signal from MatchMomentumPulse's sparse discrete markers — see §47 for why). Two `<clipPath>`s clip the SAME closed curve-to-baseline path to opposite halves for the two-tone fill, avoiding a second curve computation. SVG `<defs>` ids suffixed with `useId()` (prevents collisions across simultaneous live-feed instances). `motion.path animate={{d}}` with `initial={false}` (NOT a plain `d` prop alongside `animate` — that combination produced a real "undefined" path-data console error on first mount, caught during this sprint's own verification). Rendered `isLive`-gated, right before MatchMomentumPulse
...
│       │   │   ├── MatchStats.tsx         # Post-match statistics (possession, shots, corners, etc.). V5 Sprint 32 — `fetchMatchStats()`/`StatRow`/`STAT_MAP` extracted to `lib/matchBoxscore.ts` (second real consumer: `useMatchPressureFlow`). `StatBar`'s width fill moved from a Framer `motion.div` to a plain CSS `transition-[width]` (no compositing win from JS here); glass track behind each bar + glowing inset ring on the winning side; value spans gained `font-mono`
...
│       │   ├── useMatchPressureFlow.ts    # V5 Sprint 32 — polls `lib/matchBoxscore.ts`'s `fetchMatchStats()` every 30s, independent of whether `MatchStats`' own accordion is expanded (this is meant to be an always-visible hero element for live matches). Maintains a FIXED-length 20-sample rolling window (10 min at 30s cadence) — not cosmetic: it's the precondition for `MatchMomentumFlow`'s Framer `d`-string interpolation to morph smoothly instead of snapping, since the command structure of two consecutive `d` strings must match token-for-token
...
│       │   ├── matchBoxscore.ts            # V5 Sprint 32 — `fetchMatchStats()`/`StatRow`/`STAT_MAP` extracted from `MatchStats.tsx` (was module-private) for a second real consumer (`useMatchPressureFlow`). `computePressureSample(current, previous)` — the pressure math: a DELTA between two consecutive 30s boxscore snapshots, never raw cumulative totals (which only grow and would flatten to one value instead of showing a real swing). Hand-tuned weights (shots-on-target 3x, shots 1.5x, corners 2x, possession tilt 0.3x), clamped to [-10,+10], stated as such in a comment — same "don't let a modeled value look more scientific than it is" discipline as `GroupDistributionChart`'s Gaussian-curve note (§30)
...
│       │   ├── svgPath.ts                  # V4 Sprint 15 — smoothPath() (Catmull-Rom → cubic Bézier) extracted from Sparkline.tsx so every hand-built SVG chart shares one spline implementation. Sparkline.tsx and GroupDistributionChart.tsx both import it. V5 Sprint 32 — third real consumer, MatchMomentumFlow.tsx, no changes needed to the function itself
...
## 47. The Live Pressure Cooker — Attacking Momentum Flow (V5 Sprint 32)
