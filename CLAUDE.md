...
│       │   │   ├── Sidebar.tsx            # Desktop sidebar. `<NotificationCenter placement="right" .../>` — untouched by Sprint 23's drawer work
│       │   │   └── TopBar.tsx             # Mobile header: logo, group selector, coins, avatar. `<NotificationCenter placement="drawer" .../>` (V4 Sprint 23) — TopBar is already `sm:hidden`, so the drawer is implicitly mobile-only
│       │   ├── leaderboard/
